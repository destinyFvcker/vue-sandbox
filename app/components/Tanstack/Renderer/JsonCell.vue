<script setup lang="ts">
  ...
  const schemaToUse = ref<JsonSchema>(
    props.schema ?? Generate.jsonSchema(isObject(props.data) ? props.data : {})
  );
  ...
  watch(
    ...
    (newSchema) => {
      ...
      if (!props.uischema) {
        uischemaToUse.value = Generate.uiSchema(
          schemaToUse.value,
          undefined,
          undefined,
          schemaToUse.value
  ...
  watch(
    ...
    (newUischema) => {
      uischemaToUse.value =
        newUischema ??
        Generate.uiSchema(schemaToUse.value, undefined, undefined, schemaToUse.value);
    }
  );
