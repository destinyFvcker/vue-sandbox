<template>
	...
	>
		<slot name="loading" :loading>
			<div v-if="loading" class="bg-muted absolute inset-x-0 top-0 z-10 h-1 overflow-hidden rounded-full">
		...
		<UiTable :class="props.class">
			<UiTableHeader
				v-if="!hideHeader"
				:class="props.innerScrollHeaderSticky ? 'bg-background/90 sticky top-0 z-10 backdrop-blur-sm' : ''"
			>
										...
										<UiTooltip>
											<UiTooltipTrigger as-child>
												...
												>
													<Icon
														:name="
															row.getIsExpanded() ? expandCellIconOn : expandCellIconOff
														"
														class="size-4"
													/>
												</UiButton>
	...
	<div v-if="showFooter" class="flex flex-col gap-4 px-2 py-4 md:flex-row md:items-center md:justify-between">
...
const table = useVueTable({
	...
	onRowPinningChange: (updaterOrValue) => {
		rowPinning.value = typeof updaterOrValue === "function" ? updaterOrValue(rowPinning.value) : updaterOrValue;
...
const tableHeaderEl = computed(() => tableRootEl.value?.querySelector("thead") as HTMLElement | null);
...
const tableContainerStyle = computed<Record<string, string>>(() => {
	...
	const stickyHeaderHeight = props.innerScrollHeaderSticky && !props.hideHeader ? tableHeaderHeight.value : 0;
...
const getPinnedRowStyle = (row: Row<T>) => {
	...
	const index = typeof (row as any).getPinnedIndex === "function" ? (row as any).getPinnedIndex() : 0;
